import React from 'react';
import { Link, useLocation } from 'react-router';
import { useAuth } from '../auth/AuthProvider';

export function Navbar() {
  const { session, signOut } = useAuth();
  const location = useLocation();

  const scrollToSection = (id: string) => {
    if (location.pathname !== '/') {
      window.location.href = `/#${id}`;
      return;
    }
    const element = document.getElementById(id);
    if (element) {
      element.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  };

  return (
    <nav className="fixed top-0 w-full z-50 py-4 px-6 lg:px-12 transition-all duration-300 bg-white/40 backdrop-blur-md border-b border-white/20">
      <div className="max-w-[1200px] mx-auto flex items-center justify-between">
        
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2">
          <span className="font-sans font-bold text-xl text-[#0b1c30]">AbhyasAI</span>
        </Link>

        {/* Center Links (Only on Landing) */}
        <div className="hidden md:flex items-center gap-8">
          <button onClick={() => scrollToSection('curriculum')} className="text-[#464555] hover:text-[#4f46e5] font-medium transition-colors text-sm font-sans">
            Curriculum
          </button>
          <button onClick={() => scrollToSection('interview')} className="text-[#464555] hover:text-[#4f46e5] font-medium transition-colors text-sm font-sans">
            Interview Prep
          </button>
        </div>

        {/* Right Actions */}
        <div className="flex items-center gap-4">
          {session ? (
            <>
              <Link 
                to="/dashboard" 
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors hidden sm:block outline-none ${
                  location.pathname.startsWith('/dashboard')
                    ? 'bg-[#4f46e5]/10 text-[#4f46e5]' 
                    : 'text-[#464555] hover:bg-slate-100 hover:text-[#0b1c30]'
                }`}
              >
                Dashboard
              </Link>
              <Link 
                to="/settings" 
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors hidden sm:block outline-none ${
                  location.pathname.startsWith('/settings')
                    ? 'bg-[#4f46e5]/10 text-[#4f46e5]' 
                    : 'text-[#464555] hover:bg-slate-100 hover:text-[#0b1c30]'
                }`}
              >
                Settings
              </Link>
              <button onClick={() => signOut()} className="glass-button text-sm font-sans bg-rose-50/50 hover:bg-rose-100/50 text-rose-600 border-rose-200">
                Log Out
              </button>
            </>
          ) : (
            <>
              <Link to="/login" className="text-[#464555] hover:text-[#0b1c30] font-medium text-sm font-sans hidden sm:block">
                Log In
              </Link>
              <Link to="/signup" className="glass-button text-sm font-sans">
                Get Started
              </Link>
            </>
          )}
        </div>
      </div>
    </nav>
  );
}
